...
export default function EmployeeLoan() {
  ...
  const handleBorrow = async () => {
    ...
    try {
      ...
      if (data._id || data.loan?._id) { toast.success(`Emprunt enregistré pour ${scannedUser.fullName} !`); reset() }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)" }}>
      ...
      <div className="w-full px-4 py-8 space-y-5">
        ...
        {/* ── ÉTAPE 1 : QR LECTEUR ─── */}
        <div className={`card-p space-y-4 ${step === "user" ? "border-primary/20" : ""}`} style={step === "user" ? { borderColor: "rgba(4,8,72,0.18)" } : {}}>
